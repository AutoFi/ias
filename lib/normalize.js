...
function extractContracts(soapEnvelopeJs) {
	...
	try {
		...
		if (typeof forms['a:IElectronicProcessing.PendingElectronicContractType'] !== 'undefined') {
			forms['a:IElectronicProcessing.PendingElectronicContractType'].forEach(function(v, k) {
				var pdfData = v['a:PDFData'][0];
				var name = v['a:PDFFileName'][0];
				var contractId = v['a:PendingElectronicContractID'][0];
				var form = name.split('.')[4];

				result.contractFiles.push({name: name, data: pdfData, contractId: contractId, form: form});
			});
		}
	} catch (ex) {
		// console.log(ex);
...
function extractGetPendingElectronicContracts(soapEnvelopeJs) {
	var result = {contractFiles: []};
	try {
		var forms = soapEnvelopeJs['s:Envelope']['s:Body'][0]
			.GetPendingElectronicContractsResponse[0]
			.GetPendingElectronicContractsResult[0]
			['a:ElectronicContracts'][0]
		...
		forms.forEach(function(v, k) {
			...
			var name = v['a:PDFFileName'][0];
			var contractId = v['a:PendingElectronicContractID'][0];
			var dealNumber = v['a:DealNumber'][0];

			result.contractFiles.push({name: name, data: pdfData, contractId: contractId, dealNumber: dealNumber});
		});
	} catch (ex) {
		// console.log(ex);
		result = null;
...
function extractGenerateElectronicRemittanceBatch(soapEnvelopeJs) {
	var result = {batches: []};
	try {
		var data = soapEnvelopeJs['s:Envelope']['s:Body'][0]
			.GenerateElectronicRemittanceBatchResponse[0]
			.GenerateElectronicRemittanceBatchResult[0];
		if (data['a:ErrorInfo'][0]['a:ErrorOccurred'][0] === 'false') {
			result.success = true;
		} else {
			result.success = false;
			result.message = data['a:ErrorInfo'][0]['a:ErrorDescription'][0]
		}
		var batches = data
			['a:BatchHeaders'][0]
			['a:IElectronicProcessing.ElectronicBatchHeaderType'];

		batches.forEach(function(v) {
			result.batches.push(scrubber(v));
		});

		if (data['a:RemittanceRegisterPDF']) {
			result.pdf = data['a:RemittanceRegisterPDF'][0];
		}
	} catch (ex) {
		result = {
...
normalize.GetPendingElectronicContracts = extractGetPendingElectronicContracts;
normalize.GenerateElectronicRemittanceBatch = extractGenerateElectronicRemittanceBatch;
